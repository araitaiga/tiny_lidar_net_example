...
`_CONV_PARAMS` に (カーネル長, ストライド) のタプルを並べておき、5 層通過後の時間軸長を `_conv_out_len` で算出しています。こうしておくと、公式 Keras 実装と同じ層構成を、定義の重複なく書けます。

**FC1 の入力サイズは入力長から動的に算出している**

L (1081) / M (541) / S (271) のいずれの派生モデルでも、Conv 後のフラット長は変わります。`flat_len = 64 * self._conv_out_len(input_length)` で都度計算しているため、`TinyLidarNet(input_length=541)` のように渡せば M 版が構築されるようになっています。公式 Keras 実装は入力 shape から自動計算される性質を持っていますが、PyTorch では同等の挙動を明示的に再現する形にしました。
...
- 成功時のデモ: `outputs/Screencast from 2026-05-20 10-43-06.webm` を埋め込む予定です
...
- 失敗時のデモ: [outputs/contents/curved_training_collision.gif](tiny_lidar_net_example/outputs/contents/curved_training_collision.gif) を埋め込む予定です
...
![学習曲線](outputs/tiny_lidar_net_curved_circuit_epoch100.png)
