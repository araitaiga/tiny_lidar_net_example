...
本記事では、2D LiDAR を入力、ステアリング角と速度を出力とする End-to-End モデルである、TinyLidarNet [1] を取り上げます。TinyLidarNet は自律走行レース競技で 3 位入賞した軽量モデルで、 MCU 上でもリアルタイム推論が可能とされています。
...
そこで、TinyLidarNet では、1D の畳み込み層 (1D Conv) を含むモデルを提案しています。入力の 2D LiDAR を 1D Conv で処理することで、データに含まれる空間構造を抽出しやすくなり、走破性や未訓練コースに対する汎化性能が向上したと述べられています。  
...
入力を 5 層の 1D 畳み込み層で処理した後、4 層の全結合層で処理し、出力としてステアリング角と速度を回帰します。  
総パラメータ数は約 22 万と小規模です。そのため、学習データも小規模で済み、手動運転およそ 5 分 (約 1.2 万サンプル)の学習データで訓練して十分な性能が得られたと報告されています。
...
なお、最終層には `tanh` が入っています。 tanh は入力を [-1, 1] の範囲に揃える、S 字型の活性化関数です。
...
2 つ目は、本実装/シミュレータ特有の並び順の規約の吸収です。Control クラスでステアリングと速度の順番を合わせていますが、本質的ではありません。  


### 2.6 補足: 学習曲線

下図は約 10 分ほどの教師データを用いて、この TinyLidarNet 実装を 100 epoch 学習させた際の Huber loss の推移です。
...
また、Train Loss と Val Loss の乖離も起きておらず、比較的良好に学習が進んでいるように見えます。ただし、以下の 3.1 で確認する通り、損失そのものが収束していても、学習データ自体に偏りがあると、必ずしも新環境での汎化性能が得られるわけではないことに注意が必要です。  
...
TinyLidarNet は自己方位に対する周囲形状の情報を元に、自己方位前方側のフリーな領域へ進むような制御指令値を学習していると考えられます。そのため、初期状態を変えるだけで、ロボットがサーキットコースを時計回りに走行するか、反時計回りに走行するかが自然に切り替わります。  
...
2 モデルの平均走行継続距離 (MeanDist) と平均速度 (MeanAvgV) の比較は以下です。学習データの多様性を増やすと、モデルの汎化性能が向上し、走行継続距離が全てのケースで伸びることがわかりました。  
...
論文で主張されている、動的環境への対応能力を確認します。障害物の位置が動的に変化する World を用意し、3.2 の学習済みモデルで自動運転を行いました。  
